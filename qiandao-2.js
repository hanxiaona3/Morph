...
const main=async (privateKeys)=>{
    ...
    for (let index =0; index <privateKeys.length; index++) {//PrivateKeys$136Wallets.length
        let agent = new HttpsProxyAgent(ipURL);
        ...
        console.log(`${randTime+5}秒后执行签到`);
        await sleep(randTime+5)
        await Qiandao(wallet,headers,agent);
        //休眠
        console.log(`${randTime+6}秒后执行开盲盒`);
        await sleep(randTime+6)
        await Open_Blind_Box(wallet,headers,agent);
        console.log(`${randTime+6}秒后执行投票`);
        await sleep(randTime+6)
        //开始投票r 
...
main(NewPrivatKeys(await readTxtFile('hhplume.txt')))
